...
InputManager::Action InputManager::stringToAction(const std::string& str)
{
    ...
    if (str == "Escape")    return Action::Escape;
...
void InputManager::onActionPressed(InputManager::Action action, GameState& gameState) {

    // Guard against empty stack
    if (gameState.isEmpty())
        return;

    Screen current = gameState.top();

    // Log the pressed action with readable name
    std::cout << actionToString(action) << " pressed on screen ";
    switch (current)
    {
    case Screen::Gameplay:
        ...
        {
        case Action::Escape:
            gameState.push(Screen::PauseMenu);
            break;
        // handle other gameplay actions here if needed
        default:
            break;
        }
        break;
...
void InputManager::onActionReleased(InputManager::Action action, GameState& gameState) {
    std::cout << "button released: " << actionToString(action) << std::endl;
}

void InputManager::onActionHeld(InputManager::Action action, GameState& gameState) {
    std::cout << "button held: " << actionToString(action) << std::endl;
}

void InputManager::handleInput(const sf::Event& event, GameState& gameState)
{
    if (const auto* e = event.getIf<sf::Event::KeyPressed>())
    {
        auto it = _keyToAction.find(e->code);
        if (it != _keyToAction.end())
            onActionPressed(it->second, gameState);
    }
    else if (const auto* e = event.getIf<sf::Event::KeyReleased>())
    {
        auto it = _keyToAction.find(e->code);
        if (it != _keyToAction.end())
            onActionReleased(it->second, gameState);
    }
...
void InputManager::update(GameState& gameState) {
    ...
    for (const auto& binding : _controllerAxes)
    {
        ...
        if (value * binding.direction > 50.f)
            onActionHeld(binding.action, gameState);
    }
